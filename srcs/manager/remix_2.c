...
/*   Updated: 2022/03/31 23:17:14 by ndormoy          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((access((tmp->cmd_to_exec[0]),
				F_OK | R_OK) < 0 && previous->type == CHV_L))
		return (TRUE);
	else if (access((tmp->cmd_to_exec[0]),
			F_OK) >= 0 && (previous->type == CHV_R
